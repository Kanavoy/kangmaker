 ...
 * @this {HT.Hexagon}
 * @param {boolean} doColour whether this is the colour pass or not
 */
HT.Hexagon.prototype.draw = function(ctx, doColour) {
	if(this.colour) {
		if (!doColour)
			return
	...
	if (this.colour) {
		...
		ctx.globalAlpha = document.getElementById("shadeTerritory").value
		ctx.fill()
		ctx.globalAlpha = 1
	}
	ctx.closePath();
