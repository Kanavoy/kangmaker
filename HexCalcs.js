...
function drawHexGrid(grid)
{
	...
	woff=HT.Hexagon.Static.WIDTH, hoff=HT.Hexagon.Static.HEIGHT,
	ctx = canvas.getContext('2d'),
	img = document.getElementById("mapImg")
	ctx.clearRect(0, 0, canvas.width, canvas.height)
	ctx.drawImage(img, woff/4, 0, canvas.width-woff/2, canvas.height, 0, 0, canvas.width-woff/2, canvas.height)
	// two passes = grey first, then coloured
	for(let h in grid.Hexes)
	{
		grid.Hexes[h].draw(ctx,false)
	}	
	for(let h in grid.Hexes)
	{
		grid.Hexes[h].draw(ctx,true)
	}
	return grid
}
